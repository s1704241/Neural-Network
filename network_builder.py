...
from learning_rate_decay import cosine_decay, cosine_decay_restarts

from network_architectures import VGGClassifier, FCCLayerClassifier, Baseline, Pyramid
from weight_decay_optimisers import AdamWOptimizer


class ClassifierNetworkGraph:
    def __init__(self, input_x, target_placeholder, dropout_rate,
                 batch_size=100, num_channels=1, n_classes=100, is_training=True, augment_flag=True,
        ...
        if network_name == "VGG_classifier":
            ...
            self.c = FCCLayerClassifier(self.batch_size, name="classifier_neural_network",
                                   ...
                                   num_classes=n_classes, layer_stage_sizes=[64, 128, 256],
                                   strided_dim_reduction=strided_dim_reduction)
        elif network_name == "Baseline":
            self.c = Baseline(self.batch_size, name="classifier_neural_network",
                                   batch_norm_use=use_batch_normalization, num_channels=num_channels,
                                   num_classes=n_classes, layer_stage_sizes=[64, 128, 256],
                                   strided_dim_reduction=strided_dim_reduction)
        elif network_name == "Pyramid":
            self.c = Pyramid(self.batch_size, name="classifier_neural_network",
                                   batch_norm_use=use_batch_normalization, num_channels=num_channels,
                                   num_classes=n_classes, layer_stage_sizes=[64],
                                   strided_dim_reduction=strided_dim_reduction)

        #elif network_name == "Pyramid":
        #    self.c = Pyramid(self.batch_size, name="classifier_neural_network",
        #                           batch_norm_use=use_batch_normalization, num_channels=num_channels,
        #                           num_classes=n_classes, layer_stage_sizes=[16, 24, 32, 48],
        #                           strided_dim_reduction=strided_dim_reduction)
    ...
    def rotate_batch(self, batch_images):
        """
        flip and crop a batch of images
        :param batch_images: A batch of images
        :return: A fliped and croped batch of images (some images will not be rotated if their rotation flip ends up False)
        """
        ...
        with tf.name_scope('augment'):
            ...
            for image in batch_images_unpacked:
                new_images.append(self.preprocess_image(image))
    ...
    def data_augment_batch(self, batch_images):
        ...
        """
        batch_images = tf.cond(self.augment, lambda: self.rotate_batch(batch_images), lambda: batch_images)
        return batch_images
    ...
    def train(self, losses, learning_rate=1e-3, beta1=0.9):
      ...
      #  learning_rate=cosine_decay(learning_rate=learning_rate,global_step=0,decay_steps=33200)
        #learning_rate=cosine_decay_restarts(learning_rate=learning_rate,global_step=0,first_decay_steps=3320)
        c_opt = tf.train.AdamOptimizer(beta1=beta1, learning_rate=learning_rate)
        #c_opt = AdamWOptimizer(learning_rate=learning_rate, weight_decay=1e-5)
        #c_opt = AdamOptimizer(learning_rate=learning_rate, weight_decay=1e-5)
    ...
    def preprocess_image(self,image):
        ...
        image = tf.cond(flip_boolean, lambda: tf.image.resize_image_with_crop_or_pad(image, 32 + 8, 32 + 8),
                        lambda: image)  # if flip_boolean is true the rotate if not then do not rotate
        # Resize the image to add four extra pixels on each side.
        image = tf.cond(flip_boolean, lambda: tf.random_crop(image, [32, 32, 3]),
                        lambda: image)
